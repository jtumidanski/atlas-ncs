...
func (r Shuri) Initial(l logrus.FieldLogger, c Context) State {
	return r.HaveYouHeard(l, c)
}

func (r Shuri) HaveYouHeard(l logrus.FieldLogger, c Context) State {
	...
	return SendListSelectionExit(l, c, m.String(), r.Selection, r.OtherBusiness)
}

func (r Shuri) Selection(selection int32) StateProducer {
	switch selection {
	case 0:
		return r.Payment
	case 1:
...
func (r Shuri) Payment(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		...
		ShowMap(_map.FlorinaBeach).
		AddText(" right now?")
	return SendYesNoExit(l, c, m.String(), r.ValidatePayment, r.OtherBusiness, r.OtherBusiness)
}

func (r Shuri) UseVIP(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("So you have ").
		BlueText().ShowItemName1(item.VIPTicketToFlorinaBeach).
		...
		ShowMap(_map.FlorinaBeach).
		AddText(" right now?")
	return SendYesNoExit(l, c, m.String(), r.ValidateVIP, r.OtherBusiness, r.OtherBusiness)
}

func (r Shuri) WhatIsVIP(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Shuri) OtherBusiness(l logrus.FieldLogger, c Context) State {
...
func (r Shuri) ValidatePayment(l logrus.FieldLogger, c Context) State {
...
func (r Shuri) ValidateVIP(l logrus.FieldLogger, c Context) State {
...
func (r Shuri) CameBackWithoutIt(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendPrevious(l, c, m.String(), r.WhatIsVIP)
}

func (r Shuri) Warp(l logrus.FieldLogger, c Context) State {
	character.SaveLocation(l)(c.CharacterId, "FLORINA")
	return WarpByName(_map.FlorinaBeach, "st00")(l, c)
}

func (r Shuri) NotEnoughMeso(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Shuri) MissingTicket(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
