...
func (r SubwayExit) Initial(l logrus.FieldLogger, c Context) State {
	return r.Hello(l, c)
}

func (r SubwayExit) Hello(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("This device is connected to outside.")
	return SendNext(l, c, m.String(), r.GiveUpAndLeave)
}

func (r SubwayExit) GiveUpAndLeave(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Are you going to give up and leave this place?")
	return SendNextPrevious(l, c, m.String(), r.Confirm, r.Hello)
}

func (r SubwayExit) Confirm(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("You'll have to start from scratch the next time you come in...")
	return SendYesNo(l, c, m.String(), r.Warp, r.MaybeNextTime)
}

func (r SubwayExit) MaybeNextTime(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Alright, see you next time.")
	return SendOk(l, c, m.String())
}

func (r SubwayExit) Warp(l logrus.FieldLogger, c Context) State {
