...
func (r KiriruEllinia) Process(l logrus.FieldLogger, c Context) State {
	...
	err = npc.Warp(l)(c.WorldId, c.ChannelId, c.CharacterId, _map.EmpressRoadToEreveFromEllinia)
	if err != nil {
		l.WithError(err).Errorf("Unable to warp character %d to %d as a result of a conversation with %d.", c.CharacterId, _map.EmpressRoadToEreveFromEllinia, c.NPCId)
	}
	return Exit()(l, c)
}
