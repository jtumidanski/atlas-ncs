...
func (r Legor) Initial(l logrus.FieldLogger, c Context) State {
	...
	if !character.QuestCompleted(l)(c.CharacterId, 6924) {
...
func (r Legor) NotPassedTrials(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().AddText("You have not yet passed my trials. I can not advance you until you do so.")
	return SendOk(l, c, m.String())
}

func (r Legor) DoNotBotherMe(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().AddText("Please don't bother me right now, I am trying to concentrate.")
	return SendOk(l, c, m.String())
}

func (r Legor) Marevelous(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("You did a marvellous job passing my test. Are you ready to advance to your 4th job?")
	return SendYesNo(l, c, m.String(), r.Advance, Exit())
}

func (r Legor) IfIMust(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Legor) Advance(l logrus.FieldLogger, c Context) State {
...
func (r Legor) MakeRoom(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Legor) Selection(_ int32) StateProducer {
...
func (r Legor) ItIsDone(l logrus.FieldLogger, c Context) State {
