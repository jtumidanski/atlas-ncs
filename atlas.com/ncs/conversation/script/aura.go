...
func (r Aura) Initial(l logrus.FieldLogger, c Context) State {
...
func (r Aura) Hello(l logrus.FieldLogger, c Context) State {
...
func (r Aura) Selection(selection int32) StateProducer {
	switch selection {
	case 0:
...
func (r Aura) WhatAmISupposedToDo(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Aura) IBroughtItems(l logrus.FieldLogger, c Context) State {
...
func (r Aura) HaveLeaderBring(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Please let your leader bring the materials to me to complete this ordeal.")
	return SendNext(l, c, m.String(), Exit())
}

func (r Aura) BringFireOre(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Aura) EachMemberGetsAPiece2(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendYesNo(l, c, m.String(), r.Process(true), Exit())
}

func (r Aura) EachMemberGetsAPiece(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendYesNo(l, c, m.String(), r.Process(false), Exit())
}

func (r Aura) Process(hasAll bool) StateProducer {
...
func (r Aura) GetOut(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
...
func (r Aura) ReceivePrize(l logrus.FieldLogger, c Context) State {
	...
	return SendNext(l, c, m.String(), r.ProcessPrize)
}

func (r Aura) ProcessPrize(l logrus.FieldLogger, c Context) State {
...
func (r Aura) GotDocumentsPrize(l logrus.FieldLogger, c Context) State {
...
func (r Aura) MakeRoom(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Make sure you have room in your inventory before proceeding.")
	return SendOk(l, c, m.String())
}

func (r Aura) AlreadyReceived(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("You have already received your share. You can now exit the mines through the portal over there.")
	return SendOk(l, c, m.String())
}

func (r Aura) NoDocumentsPrize(l logrus.FieldLogger, c Context) State {
	if !event.ReceivedReward(l)(c.CharacterId) {
