...
func (r Tommie) Process(l logrus.FieldLogger, c Context) State {
	...
	err := npc.WarpById(l)(c.WorldId, c.ChannelId, c.CharacterId, _map.BeforeTakeoffToOrbisFromLeafre, 0)
	if err != nil {
		l.WithError(err).Errorf("Unable to warp character %d to %d as a result of a conversation with %d.", c.CharacterId, _map.BeforeTakeoffToOrbisFromLeafre, c.NPCId)
	}
	return Exit()(l, c)
}
