...
func (r Shawn) Initial(l logrus.FieldLogger, c Context) State {
...
func (r Shawn) Selection(selection int32) StateProducer {
	switch selection {
	case 0:
...
func (r Shawn) LiterateCivilization(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendNext(l, c, m.String(), r.LastKing)
}

func (r Shawn) LastKing(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendNextPrevious(l, c, m.String(), r.AnyMore, r.LiterateCivilization)
}

func (r Shawn) AnyMore(l logrus.FieldLogger, c Context) State {
...
func (r Shawn) LegendaryJewel(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		ShowItemName1(item.Rubian).
	...
	return SendNext(l, c, m.String(), r.UltimateGoal)
}

func (r Shawn) UltimateGoal(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendNextPrevious(l, c, m.String(), r.AnyMore, r.LegendaryJewel)
}

func (r Shawn) Before(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
	...
	return SendNext(l, c, m.String(), r.AnyMore)
}

func (r Shawn) Exit(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
