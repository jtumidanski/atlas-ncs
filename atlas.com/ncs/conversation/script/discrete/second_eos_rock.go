...
func (r SecondEOSRock) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r SecondEOSRock) ToNext(l logrus.FieldLogger, c script.Context) script.State {
...
func (r SecondEOSRock) NeedRock(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r SecondEOSRock) Confirm100(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You can use ").
		BlueText().AddText("Eos Rock Scroll").
		BlackText().AddText(" to activate ").
	...
	return script.SendYesNo(l, c, m.String(), r.Process(_map.EosTower100thFloor), script.Exit())
}

func (r SecondEOSRock) Confirm41(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You can use ").
		BlueText().AddText("Eos Rock Scroll").
		BlackText().AddText(" to activate ").
	...
	return script.SendYesNo(l, c, m.String(), r.Process(_map.EosTower41stFloor), script.Exit())
}
