...
func (r Oz) Initial(l logrus.FieldLogger, c script.Context) script.State {
	if !character.MeetsCriteria(l)(c.CharacterId, character.IsAJobCriteria(job.BlazeWizard1, job.BlazeWizard2, job.BlazeWizard3, job.BlazeWizard4)) {
		return r.NotBlazeWizard(l, c)
	}
	return r.FoundMe(l, c)
}

func (r Oz) NotBlazeWizard(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Oz) FoundMe(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Oz) Trigger(l logrus.FieldLogger, c script.Context) script.State {
