...
func (r Hikari) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText(fmt.Sprintf("Would you like to enter the bathhouse? That'll be %d mesos for you.", 300))
	return script.SendYesNo(l, c, m.String(), r.Validate, r.AnotherTime)
}

func (r Hikari) AnotherTime(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Please come back some other time.")
	return script.SendOk(l, c, m.String())
}

func (r Hikari) Validate(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Hikari) NotEnoughMesos(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText(fmt.Sprintf("Please check and see if you have %d mesos to enter this place.", 300))
	return script.SendOk(l, c, m.String())
}

func (r Hikari) Process(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.WarpByName(destination, "out00")(l, c)
}
