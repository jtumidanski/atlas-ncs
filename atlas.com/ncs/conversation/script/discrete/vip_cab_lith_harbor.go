...
func (r VIPCabLithHarbor) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.Hello(l, c)
}

func (r VIPCabLithHarbor) Hello(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText(".")
	return script.SendNextExit(l, c, m.String(), r.Cost, r.MoreToOffer)
}

func (r VIPCabLithHarbor) MoreToOffer(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r VIPCabLithHarbor) Cost(l logrus.FieldLogger, c script.Context) script.State {
...
func (r VIPCabLithHarbor) BeginnerCost(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText("?")
	return script.SendYesNoExit(l, c, m.String(), r.ValidatePayment(1000), r.MoreToOffer, r.MoreToOffer)
}

func (r VIPCabLithHarbor) RegularCost(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText("?")
	return script.SendYesNoExit(l, c, m.String(), r.ValidatePayment(10000), r.MoreToOffer, r.MoreToOffer)
}
...
func (r VIPCabLithHarbor) NotEnoughMeso(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r VIPCabLithHarbor) ProcessPayment(cost uint32) script.StateProducer {
	return func(l logrus.FieldLogger, c script.Context) script.State {
		...
		return script.WarpById(_map.AntTunnelPark, 0)(l, c)
	}
}
