...
func (r Mel) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText("?")
	return script.SendYesNo(l, c, m.String(), r.Validate, r.SomeBusiness)
}

func (r Mel) SomeBusiness(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("You must have some business to take care of here, right?")
	return script.SendOk(l, c, m.String())
}

func (r Mel) Validate(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Mel) AreYouSure(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Mel) Process(l logrus.FieldLogger, c script.Context) script.State {
