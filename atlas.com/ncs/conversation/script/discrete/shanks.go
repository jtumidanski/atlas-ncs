...
func (r Shanks) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.TakeTheShip(l, c)
}

func (r Shanks) TakeTheShip(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Shanks) No(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Hmm... I guess you still have things to do here?")
	return script.SendNext(l, c, m.String(), script.Exit())
}

func (r Shanks) Yes(l logrus.FieldLogger, c script.Context) script.State {
	if character.HasItem(l)(c.CharacterId, item.LucasRecommendationLetter) {
		...
		return script.SendNext(l, c, m.String(), r.ConfirmUse)
	} else {
		m := message.NewBuilder().
			AddText("Bored of this place? Here... Give me ").
			BoldText().AddText("150 mesos").
			NormalText().AddText(" first...")
		return script.SendNext(l, c, m.String(), r.StrongEnough)
	}
}

func (r Shanks) ConfirmUse(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, c, m.String(), r.WarpWithItem, r.Yes)
}

func (r Shanks) WarpWithItem(l logrus.FieldLogger, c script.Context) script.State {
	character.GainItem(l)(c.CharacterId, item.LucasRecommendationLetter, -1)
	return script.WarpById(_map.LithHarbor, 0)(l, c)
}

func (r Shanks) StrongEnough(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendNext(l, c, m.String(), r.WarpWithMeso)
}

func (r Shanks) WarpWithMeso(l logrus.FieldLogger, c script.Context) script.State {
