...
func (r KiruEreve) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.Hello(l, c)
}

func (r KiruEreve) Hello(l logrus.FieldLogger, c script.Context) script.State {
...
func (r KiruEreve) VictoriaIsland(l logrus.FieldLogger, c script.Context) script.State {
...
func (r KiruEreve) OhWell(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("If you're not interested, then oh well...")
	return script.SendOk(l, c, m.String())
}

func (r KiruEreve) NotEnoughMeso(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r KiruEreve) Process(l logrus.FieldLogger, c script.Context) script.State {
	err := character.GainMeso(l)(c.CharacterId, -1000)
	if err != nil {
		l.WithError(err).Errorf("Unable to process payment by character %d.", c.CharacterId)
	}
	return script.Warp(_map.EmpressRoadToOrbis)(l, c)
}
