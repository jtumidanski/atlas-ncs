...
func (r Peter) Initial(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendNext(l, c, m.String(), r.Remember)
}

func (r Peter) Remember(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("But remember, once you get out of here, you will enter a village full with monsters. Well them, good bye!")
	return script.SendNextPrevious(l, c, m.String(), r.Process, r.Initial)
}

func (r Peter) Process(l logrus.FieldLogger, c script.Context) script.State {
