...
func (r Bush4) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Bush4) PileOfBoxes(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Just a pile of boxes, nothing special...")
	return script.SendOk(l, span, c, m.String())
}

func (r Bush4) DoYouWant(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Do you want to obtain a glasses?")
	return script.SendNext(l, span, c, m.String(), r.Validate)
}

func (r Bush4) Validate(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Bush4) AlreadyHave(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You ").
...
func (r Bush4) NoInventoryRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
