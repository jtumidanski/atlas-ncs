...
func (r CamelCab) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r CamelCab) ToAriant(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText(".")
	return script.SendYesNo(l, c, m.String(), r.Validate(_map.Ariant), r.TooBusy)
}

func (r CamelCab) ToMagatia(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Would you like to take the ").
		BlueText().AddText("Camel Cab").
		BlackText().AddText(" to ").
		...
		BlueText().AddText("1500 mesos").
		BlackText().AddText(".")
	return script.SendYesNo(l, c, m.String(), r.Validate(_map.Magatia), r.TooBusy)
}
...
func (r CamelCab) ShortOnMesos(l logrus.FieldLogger, c script.Context) script.State {
...
func (r CamelCab) TooBusy(l logrus.FieldLogger, c script.Context) script.State {
