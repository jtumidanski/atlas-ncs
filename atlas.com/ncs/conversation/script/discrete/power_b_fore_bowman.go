...
func (r PowerBForeBowman) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r PowerBForeBowman) UnderLevel20(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("This training ground is available only for those under level 20.")
	return script.SendOk(l, span, c, m.String())
}

func (r PowerBForeBowman) GoToSpecial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r PowerBForeBowman) WarpSpecial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	return script.Warp(r.TrainingSpecial())(l, span, c)
}

func (r PowerBForeBowman) ChooseRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r PowerBForeBowman) RoomFull(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
