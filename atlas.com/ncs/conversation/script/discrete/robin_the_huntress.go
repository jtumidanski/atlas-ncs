...
func (r RobinTheHuntress) Initial(l logrus.FieldLogger, c script.Context) script.State {
	if c.MapId != _map.TheLovePinata {
		if c.MapId != _map.UntamedHeartsHuntingGround {
			...
			return script.SendListSelectionExit(l, c, m.String(), r.UntamedHeartsSelection, r.Goodbye)
		} else {
			m := message.NewBuilder().
				AddText("Hello, where would you like to go?").NewLine().
				OpenItem(0).BlueText().AddText("I have 7 keys. Bring me to smash boxes").CloseItem().NewLine().
				OpenItem(1).BlueText().AddText("Please warp me out.").CloseItem()
			return script.SendListSelectionExit(l, c, m.String(), r.SmashBoxesSelection, r.Goodbye)
		}
	} else {
		m := message.NewBuilder().
		...
		return script.SendListSelectionExit(l, c, m.String(), r.LovePinataSelection, r.Goodbye)
	}
}
...
func (r RobinTheHuntress) Goodbye(l logrus.FieldLogger, c script.Context) script.State {
...
func (r RobinTheHuntress) WarpToUntamedHearts(l logrus.FieldLogger, c script.Context) script.State {
	return script.WarpById(_map.UntamedHeartsHuntingGround, 0)(l, c)
}
...
func (r RobinTheHuntress) WarpToWeddingExit(l logrus.FieldLogger, c script.Context) script.State {
	return script.WarpById(_map.WeddingExitMap, 0)(l, c)
}

func (r RobinTheHuntress) ValidateUntamedHearts(l logrus.FieldLogger, c script.Context) script.State {
...
func (r RobinTheHuntress) MissingGoldenMapleLeaf(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r RobinTheHuntress) ValidateSmashBoxes(l logrus.FieldLogger, c script.Context) script.State {
...
func (r RobinTheHuntress) ProcessSmashBoxes(l logrus.FieldLogger, c script.Context) script.State {
	character.GainItem(l)(c.CharacterId, item.GoldenKey, -7)
	return script.WarpById(_map.TheLovePinata, 0)(l, c)
}

func (r RobinTheHuntress) MissingKeys(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
