...
func (r MagicianJobInstructor) TrueHero(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You're truly a hero!")
	return script.SendOk(l, c, m.String())
}

func (r MagicianJobInstructor) LetYouIn(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, c, m.String(), script.WarpById(_map.MagiciansTreeDungeon, 0))
}

func (r MagicianJobInstructor) ExplainTest(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r MagicianJobInstructor) SendYou(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r MagicianJobInstructor) AcquireMarble(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r MagicianJobInstructor) CannotLeave(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r MagicianJobInstructor) OnceYouAreReady(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
