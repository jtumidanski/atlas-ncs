...
func (r Sera) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Sera) Welcome(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendYesNo(l, c, m.String(), r.OkThen, r.ConfirmStartJourney)
}

func (r Sera) FirstTraining(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, c, m.String(), r.EntitledToAJob)
}

func (r Sera) ConfirmStartJourney(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Do you really want to start your journey right away?")
	return script.SendYesNo(l, c, m.String(), r.Skip, r.CancelSkip)
}

func (r Sera) Skip(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, c, m.String(), r.WarpSkip)
}

func (r Sera) WarpTraining(l logrus.FieldLogger, c script.Context) script.State {
	return script.WarpById(_map.UpperLevelOfTheTrainingCamp, 0)(l, c)
}

func (r Sera) WarpSkip(l logrus.FieldLogger, c script.Context) script.State {
	return script.WarpById(_map.InASmallForest, 0)(l, c)
}

func (r Sera) CancelSkip(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Please talk to me again when you finally made your decision.")
	return script.SendNext(l, c, m.String(), script.Exit())
}

func (r Sera) EntitledToAJob(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Sera) OkThen(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Ok then, I will let you enter the training camp. Please follow your instructor's lead.")
	return script.SendNext(l, c, m.String(), r.WarpTraining)
}
