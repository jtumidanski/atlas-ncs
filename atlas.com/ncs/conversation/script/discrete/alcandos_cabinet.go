...
func (r AlcandosCabinet) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if quest.IsStarted(l)(c.CharacterId, 3309) && !character.HasItem(l, span)(c.CharacterId, item.SecretDocument) {
		if character.CanHold(l)(c.CharacterId, item.SecretDocument) {
			character.GainItem(l, span)(c.CharacterId, item.SecretDocument, 1)
		} else {
