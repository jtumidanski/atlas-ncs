...
func (r ThiefJobInstructor) TrueHero(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You're truly a hero!")
	return script.SendOk(l, c, m.String())
}

func (r ThiefJobInstructor) LetYouIn(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r ThiefJobInstructor) Warp(l logrus.FieldLogger, c script.Context) script.State {
	return script.WarpById(_map.ThiefsConstructionSite, 0)(l, c)
}

func (r ThiefJobInstructor) IsntThisALetter(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r ThiefJobInstructor) ProveYourSkills(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r ThiefJobInstructor) IfYouAreReady(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r ThiefJobInstructor) OnceYouAreReady(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("I can show you the way once your ready for it.")
	return script.SendOk(l, c, m.String())
}

func (r ThiefJobInstructor) GoodLuck(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
