...
func (r Weaver) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	return script.SendYesNo(l, span, c, m.String(), r.Continue, r.TooBusy)
}

func (r Weaver) TooBusy(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Hmmm ... too busy to do it right now? If you feel like doing it, though, come back and find me.")
	return script.SendOk(l, span, c, m.String())
}

func (r Weaver) Continue(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if character.HasItem(l, span)(c.CharacterId, item.WeaversLetter) {
		return r.Info(l, span, c)
	}
	return r.HereIsTheLetter(l, span, c)
}

func (r Weaver) HereIsTheLetter(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Weaver) Info(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
