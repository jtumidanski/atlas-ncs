...
func (r KonpeiExit) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Here you are, right in front of the hideout! What? You want to").NewLine().
		AddText("return to ").
		ShowMap(_map.ShowaTown).
		AddText("?")
	return script.SendYesNo(l, span, c, m.String(), script.Warp(_map.ShowaTown), r.TalkToMe)
}

func (r KonpeiExit) TalkToMe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
