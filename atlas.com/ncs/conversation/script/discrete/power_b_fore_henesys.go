...
func (r PowerBForeHenesys) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if character.IsLevel(l, span)(c.CharacterId, 20) {
		...
		return r.GoToSpecial(l, span, c)
	} else {
		return r.ChooseRoom(l, span, c)
	}
}

func (r PowerBForeHenesys) UnderLevel20(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("This training ground is available only for those under level 20.")
	return script.SendOk(l, span, c, m.String())
}

func (r PowerBForeHenesys) GoToSpecial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Would you like to go in the special Training Center?")
	return script.SendYesNo(l, span, c, m.String(), r.WarpSpecial, script.Exit())
}

func (r PowerBForeHenesys) WarpSpecial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	return script.Warp(r.TrainingSpecial())(l, span, c)
}

func (r PowerBForeHenesys) ChooseRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r PowerBForeHenesys) RoomFull(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
