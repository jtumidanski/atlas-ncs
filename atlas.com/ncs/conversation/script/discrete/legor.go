...
func (r Legor) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !character.MeetsCriteria(l, span)(c.CharacterId, character.IsLevelCriteria(120), character.IsAJobCriteria(job.Ranger, job.BowMaster, job.Sniper, job.Marksman)) {
	...
	if character.MeetsCriteria(l, span)(c.CharacterId, character.IsAJobCriteria(job.Ranger, job.Sniper)) {
		return r.Marevelous(l, span, c)
	}
	return r.IfIMust(l, span, c)
}

func (r Legor) NotPassedTrials(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("You have not yet passed my trials. I can not advance you until you do so.")
	return script.SendOk(l, span, c, m.String())
}

func (r Legor) DoNotBotherMe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("Please don't bother me right now, I am trying to concentrate.")
	return script.SendOk(l, span, c, m.String())
}

func (r Legor) Marevelous(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Legor) IfIMust(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Legor) Advance(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !character.CanHold(l)(c.CharacterId, item.SkillBookMapleWarrior) {
		return r.MakeRoom(l, span, c)
	}
	if character.IsJob(l, span)(c.CharacterId, job.Ranger) {
		character.ChangeJob(l, span)(c.CharacterId, job.BowMaster)
		...
		character.ChangeJob(l, span)(c.CharacterId, job.Marksman)
...
func (r Legor) MakeRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Legor) Selection(_ int32) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		...
		if character.IsJob(l, span)(c.CharacterId, job.BowMaster) {
...
func (r Legor) ItIsDone(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
