...
func (r DonGiovanni) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return care.NewGenericCare(r.Hello(), []care.ChoiceConfig{r.Style(), care.ColorCareChoice(item.KerningCityHairColorCouponVIP)})(l, c)
}
...
func (r DonGiovanni) Style() care.ChoiceConfig {
	...
	vip := care.ProcessCoupon(item.KerningCityHairStyleCouponVIP, care.SetHair, care.SetSingleUse(true))
	membership := care.ProcessCoupon(item.KerningCityHairMembership, care.SetHair, care.SetSingleUse(false), care.SetFailFunction(vip))

	prompt := care.StylePrompt(item.KerningCityHairStyleCouponVIP)
	next := care.ShowChoices(prompt, choiceSupplier, membership)
	return care.NewChoiceConfig(next, care.HairStyleCouponListText(item.KerningCityHairStyleCouponVIP), care.HairStyleCouponMissing(), care.HairStyleEnjoy())
}
