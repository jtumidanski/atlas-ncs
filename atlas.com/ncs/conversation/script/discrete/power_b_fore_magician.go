...
func (r PowerBForeMagician) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r PowerBForeMagician) UnderLevel20(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("This training ground is available only for those under level 20.")
	return script.SendOk(l, span, c, m.String())
}

func (r PowerBForeMagician) ChooseRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r PowerBForeMagician) RoomFull(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
