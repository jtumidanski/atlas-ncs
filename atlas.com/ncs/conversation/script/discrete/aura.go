...
func (r Aura) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if event.Cleared(l)(c.CharacterId) {
		return r.ReceivePrize(l, span, c)
	} else {
		return r.Hello(l, span, c)
	}
}

func (r Aura) Hello(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Aura) WhatAmISupposedToDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Aura) IBroughtItems(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !event.Leader(l)(c.CharacterId) {
		return r.HaveLeaderBring(l, span, c)
	}
	if !character.HasItem(l, span)(c.CharacterId, item.FireOre) {
		return r.BringFireOre(l, span, c)
	}
	if character.HasItems(l, span)(c.CharacterId, item.PaperDocument, 30) {
		return r.EachMemberGetsAPiece2(l, span, c)
	}
	return r.EachMemberGetsAPiece(l, span, c)
}

func (r Aura) HaveLeaderBring(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Please let your leader bring the materials to me to complete this ordeal.")
	return script.SendNext(l, span, c, m.String(), script.Exit())
}

func (r Aura) BringFireOre(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Aura) EachMemberGetsAPiece2(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, span, c, m.String(), r.Process(true), script.Exit())
}

func (r Aura) EachMemberGetsAPiece(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, span, c, m.String(), r.Process(false), script.Exit())
}

func (r Aura) Process(hasAll bool) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		character.GainItem(l, span)(c.CharacterId, item.FireOre, -1)
		if hasAll {
			character.GainItem(l, span)(c.CharacterId, item.PaperDocument, -30)
...
func (r Aura) GetOut(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Aura) WarpExit(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	//TODO warp rest of party?
	return script.WarpById(_map.TheDoorToZakum, 0)(l, span, c)
}

func (r Aura) ReceivePrize(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You completed this ordeal, now receive your prize.")
	return script.SendNext(l, span, c, m.String(), r.ProcessPrize)
}

func (r Aura) ProcessPrize(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if event.GetProperty(l)(strconv.Itoa(int(c.NPCId)), "gotDocuments") == "1" {
		return r.GotDocumentsPrize(l, span, c)
	}
	return r.NoDocumentsPrize(l, span, c)
}

func (r Aura) GotDocumentsPrize(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !event.ReceivedReward(l)(c.CharacterId) {
		...
		character.GainItem(l, span)(c.CharacterId, item.ReturnScrollToDeadMine, 5)
		character.GainItem(l, span)(c.CharacterId, item.PieceOfFireOre, 1)
		event.SetRewardReceived(l)(c.CharacterId)
		return script.Exit()(l, span, c)
	}
	return r.AlreadyReceived(l, span, c)
}

func (r Aura) MakeRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Make sure you have room in your inventory before proceeding.")
	return script.SendOk(l, span, c, m.String())
}

func (r Aura) AlreadyReceived(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You have already received your share. You can now exit the mines through the portal over there.")
	return script.SendOk(l, span, c, m.String())
}

func (r Aura) NoDocumentsPrize(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !event.ReceivedReward(l)(c.CharacterId) {
