...
func (r TrainerFrod) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if character.HasItem(l, span)(c.CharacterId, item.BartosLetter) {
		return r.BrothersLetter(l, span, c)
	}
	return r.BrotherToldMe(l, span, c)
}

func (r TrainerFrod) BrothersLetter(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	return script.SendNext(l, span, c, m.String(), r.Action)
}

func (r TrainerFrod) BrotherToldMe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r TrainerFrod) Action(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if pet.HasPets(l)(c.CharacterId) {
		return r.CompleteCourse(l, span, c)
	}
	return r.CompleteCourseNoPets(l, span, c)
}

func (r TrainerFrod) CompleteCourseNoPets(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), script.Exit(), r.BrothersLetter)
}

func (r TrainerFrod) CompleteCourse(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
