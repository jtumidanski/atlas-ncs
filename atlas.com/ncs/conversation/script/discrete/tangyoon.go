...
func (r Tangyoon) SendToStable(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendNext(l, c, m.String(), r.IGetConfused)
}

func (r Tangyoon) IGetConfused(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, c, m.String(), r.Validate, r.SendToStable)
}

func (r Tangyoon) Validate(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Tangyoon) FullInventory(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("I can't give you the empty bottle because your inventory is full. Please make some room in your Etc window.")
	return script.SendOk(l, c, m.String())
}

func (r Tangyoon) Award(l logrus.FieldLogger, c script.Context) script.State {
	character.GainItem(l)(c.CharacterId, item.MilkJug, 1)
	return script.WarpById(_map.Stable, 0)(l, c)
}
