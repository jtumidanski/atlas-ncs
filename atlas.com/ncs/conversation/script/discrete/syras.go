...
func (r Syras) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText("?")
	return script.SendYesNo(l, c, m.String(), r.Validate, r.OtherBusiness)
}

func (r Syras) OtherBusiness(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You must have some business to take care of here, right?")
	return script.SendOk(l, c, m.String())
}

func (r Syras) Validate(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Syras) Process(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Syras) UnableToPurchase(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
