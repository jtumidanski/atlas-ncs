...
func (r Pison) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	m := message.NewBuilder().
		...
		BlackText().AddText(".")
	return script.SendNextExit(l, span, c, m.String(), r.Confirm(mapId), r.OtherBusiness)
}
...
func (r Pison) Confirm(mapId uint32) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		...
		return script.SendYesNoExit(l, span, c, m.String(), r.Warp(mapId), r.OtherBusiness, r.OtherBusiness)
	}
}

func (r Pison) OtherBusiness(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
