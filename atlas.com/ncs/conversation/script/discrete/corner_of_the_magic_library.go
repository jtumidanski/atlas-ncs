...
func (r CornerOfTheMagicLibrary) AngryMonsters(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("A mysterious black figure appeared and summoned a lot of angry monsters!")
	return script.SendOkTrigger(l, c, m.String(), r.Spawn)
}

func (r CornerOfTheMagicLibrary) Spawn(l logrus.FieldLogger, c script.Context) script.State {
	for i := 0; i < 10; i++ {
