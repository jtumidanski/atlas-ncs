...
func (r HiddenNote2) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().BlueText().
		AddText("(I can see something covered in grass. Should I pull it out?)")
	return script.SendYesNo(l, c, m.String(), r.Yuck, r.Nah)
}

func (r HiddenNote2) Yuck(l logrus.FieldLogger, c script.Context) script.State {
...
func (r HiddenNote2) Nah(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().BlueText().
