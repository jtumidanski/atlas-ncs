...
func (r WizTheLibrarian) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	if counter == 0 {
		return r.NoStoryBook(l, span, c)
	}
	return r.Progress(counter, cl.String())(l, span, c)
}

func (r WizTheLibrarian) NoStoryBook(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r WizTheLibrarian) Progress(counter int, books string) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		m := message.NewBuilder().
			...
			AddText(books)
		return script.SendNext(l, span, c, m.String(), r.SettlingDown(counter, books))
	}
}
...
func (r WizTheLibrarian) SettlingDown(counter int, books string) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		...
		return script.SendNextPrevious(l, span, c, m.String(), script.Exit(), r.Progress(counter, books))
	}
}
