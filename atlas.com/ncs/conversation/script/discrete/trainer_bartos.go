...
func (r TrainerBartos) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.Hello(l, c)
}

func (r TrainerBartos) Hello(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendListSelection(l, c, m.String(), r.WhatBusiness)
}
...
func (r TrainerBartos) AboutThisPlace(l logrus.FieldLogger, c script.Context) script.State {
...
func (r TrainerBartos) WordFromMar(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r TrainerBartos) WannaTrainYourPet(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, c, m.String(), r.GiveItem, r.TooBusy)
}

func (r TrainerBartos) GiveItem(l logrus.FieldLogger, c script.Context) script.State {
...
func (r TrainerBartos) TooBusy(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Hmmm ... too busy to do it right now? If you feel like doing it, though, come back and find me.")
	return script.SendNext(l, c, m.String(), script.Exit())
}

func (r TrainerBartos) GetThatLetter(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
