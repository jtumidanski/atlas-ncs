...
func (r ThomasSwift) Initial(l logrus.FieldLogger, c script.Context) script.State {
	if c.MapId == _map.Henesys {
		m := message.NewBuilder().
			AddText("I can take you to the Amoria Village. Are you ready to go?")
		return script.SendYesNo(l, c, m.String(), r.HaveAGreatTime(_map.Amoria, 0), r.HangAround)
	} else {
		m := message.NewBuilder().
			AddText("I can take you back to Henesys. Are you ready to go?")
		return script.SendYesNo(l, c, m.String(), r.HaveAGreatTime(_map.Henesys, 5), r.HangAround)
	}
}

func (r ThomasSwift) HangAround(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r ThomasSwift) HaveAGreatTime(mapId uint32, portalId uint32) script.StateProducer {
	return func(l logrus.FieldLogger, c script.Context) script.State {
		...
		return script.SendNext(l, c, m.String(), script.WarpById(mapId, portalId))
	}
}
