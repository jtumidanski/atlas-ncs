...
func (r Gaga) Initial(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendNext(l, c, m.String(), r.BossFights)
}

func (r Gaga) BossFights(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, c, m.String(), r.Confirm, r.Initial)
}

func (r Gaga) Confirm(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendAcceptDecline(l, c, m.String(), r.VeryWell, script.Exit())
}

func (r Gaga) VeryWell(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Very well. Remember, there you can assemble a team or take on the fighting on your own, it's up to you. Good luck!")
	return script.SendOkTrigger(l, c, m.String(), r.Warp)
}

func (r Gaga) Warp(l logrus.FieldLogger, c script.Context) script.State {
	character.SaveLocation(l)(c.CharacterId, "BOSSPQ")
	return script.WarpByName(_map.ExclusiveTrainingCenter, "out00")(l, c)
}
