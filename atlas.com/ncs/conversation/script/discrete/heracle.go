...
func (r Heracle) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.Hello(l, c)
}

func (r Heracle) Hello(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Heracle) AlreadyHaveGuild(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("You may not create a new Guild while you are in one.")
	return script.SendOk(l, c, m.String())
}

func (r Heracle) CreateConfirmation(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Creating a Guild costs ").
		BlueText().AddText("1500000 mesos").
		BlackText().AddText(", are you sure you want to continue?")
	return script.SendYesNo(l, c, m.String(), r.ValidateCreate, script.Exit())
}

func (r Heracle) MustBeLeaderToDisband(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("You can only disband a Guild if you are the leader of that Guild.")
	return script.SendOk(l, c, m.String())
}

func (r Heracle) DisbandConfirmation(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, c, m.String(), r.PerformDisband, script.Exit())
}

func (r Heracle) MustBeLeaderToIncrease(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You can only increase your Guild's capacity if you are the leader.")
	return script.SendOk(l, c, m.String())
}

func (r Heracle) ValidateCreate(l logrus.FieldLogger, c script.Context) script.State {
	//TODO implement
	return script.Exit()(l, c)
}

func (r Heracle) PerformDisband(l logrus.FieldLogger, c script.Context) script.State {
	//TODO implement
	return script.Exit()(l, c)
}

func (r Heracle) ValidateIncrease(l logrus.FieldLogger, c script.Context) script.State {
	//TODO implement
	return script.Exit()(l, c)
}

func (r Heracle) IncreaseConfirmation(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText(", are you sure you want to continue?")
	return script.SendYesNo(l, c, m.String(), r.ValidateIncrease, script.Exit())
}
