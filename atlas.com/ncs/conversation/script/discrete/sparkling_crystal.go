...
func (r SparklingCrystal) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("Would you like to leave?")
	return script.SendYesNo(l, c, m.String(), r.Warp, script.Exit())
}

func (r SparklingCrystal) Warp(l logrus.FieldLogger, c script.Context) script.State {
	destination := c.MapId
	switch c.MapId {
