...
func (r Spiruna) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Spiruna) GoAway(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("Go away, I'm trying to meditate.")
	return script.SendOk(l, c, m.String())
}

func (r Spiruna) RefineDarkCrystal(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendYesNo(l, c, m.String(), r.HowMany, script.Exit())
}

func (r Spiruna) HowMany(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("Okay, so how many do you want me to make?")
	return script.SendGetNumber(l, c, m.String(), r.Validate, 1, 1, 100)
}
...
func (r Spiruna) NotForFree(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("I'm sorry, but I am NOT doing this for free.")
	return script.SendOk(l, c, m.String())
}

func (r Spiruna) MoreOre(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("I need that ore to refine the Crystal. No exceptions..")
	return script.SendOk(l, c, m.String())
}

func (r Spiruna) MoreInventorySpace(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Spiruna) Success(l logrus.FieldLogger, c script.Context) script.State {
