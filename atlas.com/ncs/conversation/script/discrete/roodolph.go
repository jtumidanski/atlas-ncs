...
func (r Roodolph) Initial(l logrus.FieldLogger, c script.Context) script.State {
	if c.MapId == _map.Happyville {
		...
		return script.SendYesNo(l, c, m.String(), script.WarpById(_map.ExtraFrostySnowZone, 0), r.WhenYouWantTo)
	} else if c.MapId == _map.ExtraFrostySnowZone {
		m := message.NewBuilder().
			AddText("Do you wish to return to Happyville?")
		return script.SendYesNo(l, c, m.String(), script.WarpById(_map.Happyville, 0), r.WhenYouWantTo)
	}
	m := message.NewBuilder().AddText("You Alright?")
	return script.SendOk(l, c, m.String())
}

func (r Roodolph) WhenYouWantTo(l logrus.FieldLogger, c script.Context) script.State {
