...
func (r Simon) Initial(l logrus.FieldLogger, c script.Context) script.State {
	if c.MapId == _map.Happyville {
		m := message.NewBuilder().
			...
			BlackText().AddText("?")
		return script.SendYesNo(l, c, m.String(), script.WarpById(_map.ShalomTemple,0), r.LetMeKnow)
	} else if c.MapId == _map.ShalomTemple {
		m := message.NewBuilder().
			AddText("Would you like to head back to Happyville?")
		return script.SendYesNo(l, c, m.String(), script.WarpById(_map.Happyville, 0), r.LetMeKnow)
	}
	return script.Exit()(l, c)
}

func (r Simon) LetMeKnow(l logrus.FieldLogger, c script.Context) script.State {
