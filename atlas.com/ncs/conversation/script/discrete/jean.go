...
func (r Jean) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	return script.SendNext(l, span, c, m.String(), r.WhatShouldIDo)
}

func (r Jean) WhatShouldIDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.GoWithMe, r.Initial)
}

func (r Jean) GoWithMe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Selection(selection int32) script.StateProducer {
	switch selection {
	case 0:
		return r.Event
	case 1:
...
func (r Jean) Event(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Explanation(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Game(selection int32) script.StateProducer {
	switch selection {
	case 0:
		return r.OlaOla
	case 1:
...
func (r Jean) OlaOla(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Fitness(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Snowball(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Coconut(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) OxQuiz(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) TreasureHunt(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Jean) Go(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	ok := event.JoinEvent(l)(c.WorldId, c.ChannelId, c.CharacterId)
	if ok {
		return script.Exit()(l, span, c)
	} else {
		return r.NotYetStarted(l, span, c)
	}
}

func (r Jean) NotYetStarted(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
