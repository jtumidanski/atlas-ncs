...
func (r Cesar3) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Cesar3) HugeFestival(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText(".")
	return script.SendNext(l, c, m.String(), r.Explained)
}

func (r Cesar3) Sorry(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("You're not between level 20 and 30. Sorry, you may not participate.")
	return script.SendOk(l, c, m.String())
}

func (r Cesar3) Explained(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, c, m.String(), r.HugeFestival, r.AreYouInterested)
}

func (r Cesar3) AreYouInterested(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Cesar3) Warp(l logrus.FieldLogger, c script.Context) script.State {
	character.SaveLocation(l)(c.CharacterId, "MIRROR")
	return script.WarpById(_map.BattleArenaLobby, 3)(l, c)
}
