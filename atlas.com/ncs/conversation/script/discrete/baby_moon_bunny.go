...
func (r BabyMoonBunny) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r BabyMoonBunny) Validate(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !character.IsLevel(l, span)(c.CharacterId, 12) {
		return r.LevelRequirement(l, span, c)
	}
	return r.WhatDoWeDo(l, span, c)
}

func (r BabyMoonBunny) WhatDoWeDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, span, c, m.String(), r.ThankYou, script.Exit())
}

func (r BabyMoonBunny) LevelRequirement(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r BabyMoonBunny) ThankYou(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Thank you so much. Please rescue Gaga! Grandpa from the moon will help you.")
	return script.SendNext(l, span, c, m.String(), script.WarpById(_map.MoonCorner, 0))
}
