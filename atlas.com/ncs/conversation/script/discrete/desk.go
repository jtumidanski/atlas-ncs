...
func (r Desk) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	if quest.IsStarted(l)(c.CharacterId, 3322) && !character.HasItem(l, span)(c.CharacterId, item.SilverPendant) {
		if character.CanHold(l)(c.CharacterId, item.SilverPendant) {
			character.GainItem(l, span)(c.CharacterId, item.SilverPendant, 1)
		} else {
