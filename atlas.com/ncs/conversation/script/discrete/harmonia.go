...
func (r Harmonia) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !character.MeetsCriteria(l, span)(c.CharacterId, character.IsLevelCriteria(120), character.IsAJobCriteria(job.DragonKnight, job.Crusader, job.WhiteKnight, job.Hero, job.Paladin, job.DarkKnight)) {
	...
	if character.MeetsCriteria(l, span)(c.CharacterId, character.IsAJobCriteria(job.DragonKnight, job.Crusader, job.WhiteKnight)) {
		return r.Marevelous(l, span, c)
	}
	return r.IfIMust(l, span, c)
}

func (r Harmonia) NotPassedTrials(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("You have not yet passed my trials. I can not advance you until you do so.")
	return script.SendOk(l, span, c, m.String())
}

func (r Harmonia) DoNotBotherMe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("Please don't bother me right now, I am trying to concentrate.")
	return script.SendOk(l, span, c, m.String())
}

func (r Harmonia) Marevelous(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r Harmonia) IfIMust(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Harmonia) Advance(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if !character.CanHold(l)(c.CharacterId, item.SkillBookMapleWarrior) {
		return r.MakeRoom(l, span, c)
	}
	if character.IsJob(l, span)(c.CharacterId, job.Crusader) {
		character.ChangeJob(l, span)(c.CharacterId, job.Hero)
	...
	} else if character.IsJob(l, span)(c.CharacterId, job.WhiteKnight) {
		character.ChangeJob(l, span)(c.CharacterId, job.Paladin)
		...
		character.ChangeJob(l, span)(c.CharacterId, job.DarkKnight)
...
func (r Harmonia) MakeRoom(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Harmonia) Selection(_ int32) script.StateProducer {
	return func(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
		...
		if character.IsJob(l, span)(c.CharacterId, job.Hero) {
		...
		} else if character.IsJob(l, span)(c.CharacterId, job.Paladin) {
...
func (r Harmonia) ItIsDone(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
