...
func (r BranchSnowman) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("We have a beautiful christmas tree.").NewLine().
		AddText("Do you want to see/decorate it?")
	return script.SendYesNo(l, span, c, m.String(), script.WarpById(_map.TheHillOfChristmas1, 0), script.Exit())
}