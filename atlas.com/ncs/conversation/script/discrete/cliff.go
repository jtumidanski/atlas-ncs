...
func (r Cliff) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	return script.SendNext(l, span, c, m.String(), r.TreeRules)
}

func (r Cliff) TreeRules(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.NoExpiration, r.Initial)
}

func (r Cliff) NoExpiration(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.BuyOrnaments, r.TreeRules)
}

func (r Cliff) BuyOrnaments(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendPrevious(l, span, c, m.String(), r.NoExpiration)
}
