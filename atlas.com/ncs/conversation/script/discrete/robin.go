...
func (r Robin) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	return r.AskMeAnything(l, span, c)
}

func (r Robin) AskMeAnything(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText()
	return script.SendListSelection(l, span, c, m.String(), r.ProcessListSelection)
}

func (r Robin) ProcessListSelection(selection int32) script.StateProducer {
	switch selection {
	case 0:
		return r.HowToMove
	case 1:
...
func (r Robin) HowToMove(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, span, c, m.String(), r.AttackMonsters)
}

func (r Robin) HowToTakeDownMonster(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("Here's how to take down a monster. Every monster possesses an HP of its own and you'll take them down by attacking with either a weapon or through spells. Of course the stronger they are, the harder it is to take them down.")
	return script.SendNext(l, span, c, m.String(), r.JobAdvancement)

}

func (r Robin) HowToGather(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, span, c, m.String(), r.FullInventory)
}

func (r Robin) WhenYouDie(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("Curious to find out what happens when you die? You'll become a ghost when your HP reaches 0. There will be a tombstone in that place and you won't be able to move, although you still will be able to chat.")
	return script.SendNext(l, span, c, m.String(), r.BeginnerDeath)

}

func (r Robin) ChoosingAJob(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("When do you get to choose your job? Hahaha, take it easy, my friend. Each job has a requirement set for you to meet. Normally a level between 8 and 10 will do, so work hard.")
	return script.SendNext(l, span, c, m.String(), r.HowToAdvance)

}

func (r Robin) TheIsland(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("Want to know about this island? It's called Maple Island and it floats in the air. It's been floating in the sky for a while so the nasty monsters aren't really around. It's a very peaceful island, perfect for beginners!")
	return script.SendNext(l, span, c, m.String(), r.PowerfulPlayer)

}

func (r Robin) WarriorToDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, span, c, m.String(), r.AskMeAnything)

}

func (r Robin) BowmanToDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Robin) MagicianToDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You want to become a ").
	...
	return script.SendNext(l, span, c, m.String(), r.MagicianSpecial)

}

func (r Robin) ThiefToDo(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("You want to become a ").
...
func (r Robin) RaiseStats(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNext(l, span, c, m.String(), r.AbilityExplanation)

}

func (r Robin) CheckItems(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Robin) WearItems(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Robin) CheckEquipment(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Robin) SpecialAbilities(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r Robin) GetToVictoria(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("How do you get to Victoria Island? On the east of this island there's a harbor called Southperry. There, you'll find a ship that flies in the air. In front of the ship stands the captain. Ask him about it.")
	return script.SendNext(l, span, c, m.String(), r.OneLastPiece)

}

func (r Robin) Mesos(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().AddText("It's the currency used in MapleStory. You may purchase items through mesos. To earn them, you may either defeat the monsters, sell items at the store, or complete quests...")
	return script.SendNext(l, span, c, m.String(), r.AskMeAnything)

}

func (r Robin) AttackMonsters(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.HowToMove)
}

func (r Robin) JobAdvancement(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.HowToTakeDownMonster)
}

func (r Robin) FullInventory(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.HowToGather)
}

func (r Robin) BeginnerDeath(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.WhenYouDie)
}

func (r Robin) HowToAdvance(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.ChoosingAJob)
}

func (r Robin) PowerfulPlayer(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.TheIsland)
}

func (r Robin) MagicianSpecial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.MagicianToDo)
}

func (r Robin) AbilityExplanation(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.RaiseStats)
}

func (r Robin) OneLastPiece(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendNextPrevious(l, span, c, m.String(), r.AskMeAnything, r.GetToVictoria)
}
