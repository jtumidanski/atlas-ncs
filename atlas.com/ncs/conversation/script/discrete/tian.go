...
func (r Tian) Initial(l logrus.FieldLogger, c script.Context) script.State {
...
func (r Tian) NeedTicket(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Make sure you got a Orbis ticket to travel in this train. Check your inventory.")
	return script.SendOk(l, c, m.String())
}

func (r Tian) AlreadyTraveling(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("The train to Orbis is already travelling, please be patient for the next one.")
	return script.SendOk(l, c, m.String())
}

func (r Tian) Confirm(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Do you want to go to Orbis?")
	return script.SendYesNo(l, c, m.String(), r.Validate, r.TalkToMe)
}

func (r Tian) TalkToMe(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("Okay, talk to me if you change your mind!")
	return script.SendOk(l, c, m.String())
}

func (r Tian) Validate(l logrus.FieldLogger, c script.Context) script.State {
	if !character.TransportBoarding(l)(c.CharacterId, _map.StationOrbis, _map.OrbisStationEntrance) {
...
func (r Tian) BePatient(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().AddText("The train to Orbis is ready to take off, please be patient for the next one.")
	return script.SendOk(l, c, m.String())
}

func (r Tian) Warp(l logrus.FieldLogger, c script.Context) script.State {
	character.GainItem(l)(c.CharacterId, item.TicketToOrbisRegular, -1)
	return script.WarpById(_map.BeforeTheDepartureOrbis, 0)(l, c)
}
