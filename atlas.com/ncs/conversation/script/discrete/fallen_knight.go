...
func (r FallenKnight) Initial(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		...
		BlackText().AddText("!")
	return script.SendNext(l, c, m.String(), r.Ready)
}

func (r FallenKnight) Ready(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r FallenKnight) Validate(l logrus.FieldLogger, c script.Context) script.State {
	if _map.CharacterCount(l)(c.WorldId, c.ChannelId, _map.QuietEreve) > 0 {
		return r.AlreadyChallenging(l, c)
	}
	return script.WarpById(_map.QuietEreve, 0)(l, c)
}

func (r FallenKnight) AlreadyChallenging(l logrus.FieldLogger, c script.Context) script.State {
