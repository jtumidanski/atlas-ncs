...
func (r BlockedEntrance) Initial(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	if c.MapId == _map.TheLastCastleTower {
		if !quest.IsCompleted(l)(c.CharacterId, 2331) {
			return script.Exit()(l, span, c)
		}
		return r.ShowBossFights(l, span, c)
	}
	return r.LimitedSelection(l, span, c)
}

func (r BlockedEntrance) ShowBossFights(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r BlockedEntrance) KingPepe(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
...
func (r BlockedEntrance) PrimeMinister(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	if !ok {
		return r.AnotherParty(l, span, c)
	}
	return script.Exit()(l, span, c)
}

func (r BlockedEntrance) AnotherParty(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("Another party is already challenging the boss in this channel.")
	return script.SendOk(l, span, c, m.String())
}

func (r BlockedEntrance) LimitedSelection(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	...
	return r.ShowBossFight(l, span, c)
}

func (r BlockedEntrance) ShowBossFight(l logrus.FieldLogger, span opentracing.Span, c script.Context) script.State {
	m := message.NewBuilder().
		OpenItem(0).AddText("Enter to fight ").
		BlueText().AddText("King Pepe").
		BlackText().AddText(" and ").
