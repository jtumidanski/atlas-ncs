...
func (r TheTicketGate) Initial(l logrus.FieldLogger, c script.Context) script.State {
	return r.Hello(l, c)
}

func (r TheTicketGate) Hello(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendListSelection(l, c, m.String(), r.PickDestination)
}
...
func (r TheTicketGate) KerningSquare(l logrus.FieldLogger, c script.Context) script.State {
...
func (r TheTicketGate) ConstructionSite(l logrus.FieldLogger, c script.Context) script.State {
	...
	return script.SendListSelection(l, c, m.String(), r.PickConstructionTicket)
}

func (r TheTicketGate) NewLeafCity(l logrus.FieldLogger, c script.Context) script.State {
...
func (r TheTicketGate) AlreadyFull(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("The passenger wagon is already full. Try again a bit later.")
	return script.SendOk(l, c, m.String())
}

func (r TheTicketGate) NeedATicket(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r TheTicketGate) BuyATicketFromBell(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
		AddText("It seems you don't have a ticket! You can buy one from Bell.")
	return script.SendOk(l, c, m.String())
}

func (r TheTicketGate) BePatient(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
...
func (r TheTicketGate) DoYouWantToGetOn(l logrus.FieldLogger, c script.Context) script.State {
	m := message.NewBuilder().
	...
	return script.SendYesNo(l, c, m.String(), r.ProcessBoarding, script.Exit())
}

func (r TheTicketGate) ProcessBoarding(l logrus.FieldLogger, c script.Context) script.State {
	if !character.TransportBoarding(l)(c.CharacterId, _map.SubwayTicketingBooth, _map.NLCSubwayStation) {
		return r.BePatient(l, c)
	}
	character.GainItem(l)(c.CharacterId, item.SubwayTicketToNLCRegular, -1)
	return script.WarpById(_map.WaitingRoomFromKerningCityToNewLeafCity, 0)(l, c)
}
