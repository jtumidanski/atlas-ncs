package script

import (
	"atlas-ncs/character"
	"atlas-ncs/job"
	"atlas-ncs/monster"
	"atlas-ncs/npc"
	"atlas-ncs/npc/message"
	"github.com/sirupsen/logrus"
)

type Oz struct {
}

func (r Oz) NPCId() uint32 {
	return npc.Oz
}

func (r Oz) Initial(l logrus.FieldLogger, c Context) State {
	if !character.MeetsCriteria(l)(c.CharacterId, character.IsAJobCriteria(job.BlazeWizard1, job.BlazeWizard2, job.BlazeWizard3, job.BlazeWizard4)) {
		return r.NotBlazeWizard(l, c)
	}
	return r.FoundMe(l, c)
}

func (r Oz) NotBlazeWizard(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Hello there, ").ShowCharacterName().
		AddText(". Are you helping us finding the intruder? He is not in this area, I've already searched here.")
	return SendOk(l, c, m.String())
}

func (r Oz) FoundMe(l logrus.FieldLogger, c Context) State {
	m := message.NewBuilder().
		AddText("Darn, you found me! Then, there's only one way out! Let's fight, like ").
		RedText().AddText("Black Wings").
		BlackText().AddText(" should!")
	return SendOkTrigger(l, c, m.String(), r.Trigger)
}

func (r Oz) Trigger(l logrus.FieldLogger, c Context) State {
	monster.SpawnMonsterOnNPC(l)(c.WorldId, c.ChannelId, c.MapId, monster.MasterOfDisguise, c.NPCObjectId)
	npc.Destroy(l)(c.WorldId, c.ChannelId, c.MapId, c.NPCId)
	return Exit()(l, c)
}