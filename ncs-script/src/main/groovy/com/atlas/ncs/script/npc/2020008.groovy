...
class NPC2020008 {
   ...
   def start() {
      if (cm.isQuestStarted(6192)) {
         if (cm.getParty().isEmpty()) {
            cm.sendOk("2020008_FORM_A_PARTY")
            cm.dispose()
            return
         }
         ...
         if (eli.size() > 0) {
            ...
            cm.sendOk("2020008_PARTY_REQUIREMENTS_UNMET")
         }
      ...
      cm.sendSimple("2020008_CAN_I_HELP_YOU", cm.getJobId() % 10 == 0 ? "\r\n#L0#I want to make the 3th job advancement." : "")
   }
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (mode == 0 && type == 0) {
         ...
         if (mode == 0 && type == 1) {
            cm.sendNext("2020008_MAKE_UP_YOUR_MIND")
         }
         cm.dispose()
         return
      }
      if (action["Mental"]) {
         if (status == 0) {
            cm.sendNext("2020008_GREAT_JOB_MENTAL")
         } else if (status == 1) {
            cm.sendYesNo("2020008_SP_MUST_BE_SPENT")
         } else if (status == 2) {
            if (cm.getRemainingSp() > 0) {
               if (cm.getRemainingSp() > (cm.getLevel() - 70) * 3) {
                  cm.sendNext("2020008_USE_ALL_SP")
                  cm.dispose()
                  return
               }
            }
            ...
            if (Math.floor(cm.getJobId() / 10) == 11) {
               cm.sendNext("2020008_CRUSADER_SUCCESS")
            } else if (Math.floor(cm.getJobId() / 10) == 12) {
               cm.sendNext("2020008_WHITE_KNIGHT_SUCCESS")
            } else {
               cm.sendNext("2020008_DRAGON_KNIGHT_SUCCESS")
            }
         } else if (status == 3) {
            cm.sendNextPrev("2020008_GIVEN_SP_AND_AP")
         }
      } else if (action["Physical"]) {
         if (status == 0) {
            cm.sendNext("2020008_GREAT_JOB_PHYSICAL")
         } else if (status == 1) {
            if (cm.haveItem(4031057)) {
               cm.gainItem(4031057, (short) -1)
               cm.setPartyQuestItemObtained("JBQ")
            }
            cm.sendNextPrev("2020008_2ND_HALF")
         } else if (status == 2) {
            cm.sendNextPrev("2020008_ANSWER_EACH_AND_EVERY_QUESTION")
         }
      } else if (cm.gotPartyQuestItem("JB3") && selection == 0) {
         cm.sendNext("2020008_GO_TALK_WITH")
         cm.dispose()
      } else if (cm.gotPartyQuestItem("JBQ") && selection == 0) {
         cm.sendNext("2020008_GO_TALK_WITH_2")
         cm.dispose()
      } else {
         if (sel == -1) {
            sel = selection
         ...
         if (sel == 0) {
            if (cm.getLevel() >= 70 && cm.getJobId() % 10 == 0) {
               if (status == 0) {
                  cm.sendYesNo("2020008_WELCOME")
               } else if (status == 1) {
                  cm.setPartyQuestItemObtained("JB3")
                  cm.sendNext("2020008_TESTED_STRENGTH_AND_WISDOM")
               } else if (status == 2) {
                  cm.sendNextPrev("2020008_MENTAL_AFTER_PHYSICAL")
               }
            }
         } else {
            if (cm.getLevel() >= 50) {
               cm.sendOk("2020008_GOOD_LUCK")
               ...
               cm.sendOk("2020008_TOO_WEAK_FOR_ZAKUM")
            }
            cm.dispose()
         }
      }
