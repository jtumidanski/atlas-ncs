...
class NPC2020011 {
   ...
   def start() {
      ...
      cm.sendSimple("2020011_ANYTHING_YOU_WANT_FROM_ME", cm.getJobId() % 10 == 0 ? "\r\n#L0#I want to make the 3th job advancement." : "")
   }
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (mode == 0 && type == 0) {
         ...
         if (mode == 0 && type == 1) {
            cm.sendNext("2020011_MAKE_UP_YOUR_MIND")
         }
         cm.dispose()
         return
      }
      if (action["Mental"]) {
         if (status == 0) {
            cm.sendNext("2020011_GREAT_JOB_MENTAL")
         } else if (status == 1) {
            cm.sendYesNo("2020011_OKAY")
         } else if (status == 2) {
            if (cm.getRemainingSp() > 0) {
               if (cm.getRemainingSp() > (cm.getLevel() - 70) * 3) {
                  cm.sendNext("2020011_USE_ALL_SP")
                  cm.dispose()
                  return
               }
            }
            ...
            if (Math.floor(cm.getJobId() / 10) == 41) {
               cm.sendNext("2020011_HERMIT_SUCCESS")
            } else {
               cm.sendNext("2020011_CHIEF_BANDIT_SUCCESS")
            }

         } else if (status == 3) {
            cm.sendNextPrev("2020011_GIVEN_SP_AND_AP")
         }
      } else if (action["Physical"]) {
         if (status == 0) {
            cm.sendNext("2020011_GREAT_JOB_PHYSICAL")
         } else if (status == 1) {
            if (cm.haveItem(4031057)) {
               cm.gainItem(4031057, (short) -1)
               cm.setPartyQuestItemObtained("JBQ")
            }
            cm.sendNextPrev("2020011_2ND_HALF")
         } else if (status == 2) {
            cm.sendNextPrev("2020011_ANSWER_EACH_AND_EVERY_QUESTION")
         }
      } else if (cm.gotPartyQuestItem("JB3") && selection == 0) {
         cm.sendNext("2020011_GO_TALK_WITH")
         cm.dispose()
      } else if (cm.gotPartyQuestItem("JBQ") && selection == 0) {
         cm.sendNext("2020011_GO_TALK_WITH_2")
         cm.dispose()
      } else {
         if (sel == -1) {
            sel = selection
         ...
         if (sel == 0) {
            if (cm.getLevel() >= 70 && cm.getJobId() % 10 == 0) {
               if (status == 0) {
                  cm.sendYesNo("2020011_WELCOME")
               } else if (status == 1) {
                  cm.setPartyQuestItemObtained("JB3")
                  cm.sendNext("2020011_TEST_STRENGTH_AND_WISDOM")
               } else if (status == 2) {
                  cm.sendNextPrev("2020011_MENTAL_AFTER_PHYSICAL")
               }
            }
         } else {
            if (cm.getLevel() >= 50) {
               cm.sendOk("2020011_GOOD_LUCK")
               ...
               cm.sendOk("2020011_TOO_WEAK_FOR_ZAKUM")
            }
            cm.dispose()
         }
      }
...
NPC2020011 getNPC() {
   if (!getBinding().hasVariable("npc")) {
      ...
      getBinding().setVariable("npc", new NPC2020011(cm: cm))
   }
   return (NPC2020011) getBinding().getVariable("npc")
}
