   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (cm.getMapId() != 261000021) {
            ...
            if (status == 0) {
               em = cm.getEventManager("MagatiaPQ_A").orElseThrow()
               if (em == null) {
               ...
               cm.sendSimple("2112003_PARTY_QUEST_INFO", em.getProperty("party"), cm.isRecvPartySearchInviteEnabled() ? "disable" : "enable")
