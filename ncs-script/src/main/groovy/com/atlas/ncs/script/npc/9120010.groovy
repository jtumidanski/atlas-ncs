...
class NPC9120010 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (status == 0) {
            cm.sendYesNo("9120010_HEAR_MY_STORY")
            ...
            if (!cm.canHold(prizeItem)) {
               cm.sendNext("9120010_INVENTORY_FULL")
               ...
               cm.sendOk("9120010_ONLY_DEEM_STANDARD", prizeItem)
            } else {
               cm.sendOk("9120010_WHAT_ARE_YOU_DOING")
            }
            cm.dispose()
         }
      }
