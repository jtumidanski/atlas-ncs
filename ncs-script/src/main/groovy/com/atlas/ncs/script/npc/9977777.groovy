...
class NPC9977777 {
   ...
   String anthemSong = "Field/anthem/brazil"     // sound src: https://c7.rbxcdn.com/f91060652a6e9fbfbf92cb1418435448
   String ambientSong = "Bgm04/Shinin'Harbor"

   String[][] feature_tree = []
   String[] feature_cursor
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (status == 0) {
            ...
            String sendStr = "#b" + tabs[selection] + "#k:\r\n\r\n"
   ...
   def addFeature(String feature) {
      feature_cursor << feature
   }

   def writeFeatureTab_PQs() {
   ...
   def writeFeatureTab_MonstersMapsReactors() {
   ...
   def writeFeatureTab_PQpotentials() {
   ...
   def writeFeatureTab_Playerpotentials() {
   ...
   def writeFeatureTab_Serverpotentials() {
   ...
   def writeFeatureTab_CustomNPCs() {
   ...
   def writeAllFeatures() {
      for (int i = 0; i < tabs.length; i++) {
         feature_cursor = []

         String tabName = (tabs[i]).replaceAll(/[ ,&\/]+/, "")
         GroovyObject.invokeMethod("writeFeatureTab_" + tabName, "")

         feature_tree << feature_cursor
      }
   }
}
...
NPC9977777 getNPC() {
   if (!getBinding().hasVariable("npc")) {
      ...
      getBinding().setVariable("npc", new NPC9977777(cm: cm))
   }
   return (NPC9977777) getBinding().getVariable("npc")
}
