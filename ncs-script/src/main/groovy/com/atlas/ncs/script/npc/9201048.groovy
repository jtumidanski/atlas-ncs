...
class NPC9201048 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (status == 0) {
            em = cm.getEventManager("AmoriaPQ").orElseThrow()
            if (em == null) {
            ...
            cm.sendSimple("9201048_PARTY_QUEST_INFO", em.getProperty("party"), cm.isRecvPartySearchInviteEnabled() ? "disable" : "enable")
