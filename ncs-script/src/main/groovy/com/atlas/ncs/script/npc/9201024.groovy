   ...
   def processNanaQuest() {
      if (cm.haveItem(questItems[nanaLoc], 50)) {
         if (cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk("9201024_THANK_YOU")
            return true
         } else {
            cm.sendOk("9201024_ETC_SPACE_NEEDED")
         }
      } else {
         cm.sendOk("9201024_GATHER_ME", questItems[nanaLoc])
      }
         ...
         if (status == 0) {
            ...
            if (!cm.haveItem(4031367 + nanaLoc, 1)) {
               if (cm.isQuestCompleted(100401 + nanaLoc)) {
                  state = 1
                  cm.sendAcceptDecline("9201024_DID_YOU_LOSE", questItems[nanaLoc])
                  ...
                  state = 0
                  cm.sendAcceptDecline("9201024_SEARCHING_FOR")

               }
            } else {
               cm.sendOk("9201024_HEY_THERE")
            ...
            if (state == 0) {
               ...
               cm.sendOk("9201024_COLLECT", questItems[nanaLoc])
               cm.dispose()
            } else {
               processNanaQuest()
               cm.dispose()
...
NPC9201024 getNPC() {
   if (!getBinding().hasVariable("npc")) {
      ...
      getBinding().setVariable("npc", new NPC9201024(cm: cm))
   }
   return (NPC9201024) getBinding().getVariable("npc")
}
