   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (eim.getProperty(stage.toString() + "stageclear") != null) {
            cm.sendNext("2040038_GO_TO_NEXT_STAGE")
         } else {
            if (eim.isEventLeader(cm.getPlayer())) {
               int state = eim.getIntProperty("statusStg" + stage)

               if (state == -1) {           // preamble
                  cm.sendOk("2040038_WELCOME", stage)
                  eim.setProperty("statusStg" + stage, 0)
               } else {       // check stage completion
                  if (cm.haveItem(4001022, 32)) {
                     cm.sendOk("2040038_GOOD_JOB")
                     cm.gainItem(4001022, (short) -32)

                     eim.setProperty("statusStg" + stage, 1)
                     clearStage(stage, eim, curMap)
                  } else {
                     cm.sendNext("2040038_SORRY")
                  }
               }
            } else {
               cm.sendNext("2040038_PARTY_LEADER_MUST_TALK")
            }
         }
...
NPC2040038 getNPC() {
   if (!getBinding().hasVariable("npc")) {
      ...
      getBinding().setVariable("npc", new NPC2040038(cm: cm))
   }
   return (NPC2040038) getBinding().getVariable("npc")
}
