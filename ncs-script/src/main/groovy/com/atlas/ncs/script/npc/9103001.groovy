...
class NPC9103001 {
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (status == 0) {
            em = cm.getEventManager("LudiMazePQ").orElseThrow()
            if (em == null) {
            ...
            cm.sendSimple("9103001_PARTY_QUEST_INFO", em.getProperty("party"), cm.isRecvPartySearchInviteEnabled() ? "disable" : "enable")
