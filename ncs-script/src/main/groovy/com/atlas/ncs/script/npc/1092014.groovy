...
class NPC1092014 {
   ...
   def start() {
      cm.sendNext("1092014_NAUTILUS_TAXI_HELLO")
   }
   ...
   def action(Byte mode, Byte type, Integer selection) {
      if (mode == -1) {
         ...
         if (status == 1 && mode == 0) {
            ...
            cm.sendNext("1092014_A_LOT_TO_SEE")
            cm.dispose()
            return
         }
         if (mode == 1) {
         ...
         if (status == 1) {
            ...
            cm.sendYesNo("1092014_NOTHING_ELSE_TO_DO", maps[selection], (cm.getJobId() == 0 ? cost[selection] / 10 : cost[selection]))
            ...
            if (cm.getMeso() < mesos) {
               cm.sendNext("1092014_NOT_ENOUGH_MESOS")
               cm.dispose()
               return
            }
