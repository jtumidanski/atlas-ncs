...
class NPC2083004 {
   ...
   EventManager em
   ExpeditionType expeditionType = ExpeditionType.HORNTAIL
   String expeditionName = "Horntail"
   String expeditionBoss = "mighty Horntail"
   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      em = cm.getEventManager("HorntailBattle").orElseThrow()
      ...
      if (mode == -1) {
         ...
         if (status == 0) {
            if (cm.getLevel() < expeditionType.getMinLevel() || cm.getLevel() > expeditionType.getMaxLevel()) {
               cm.sendOk("2083004_DO_NOT_MEET_CRITERIA", expeditionBoss)

               cm.dispose()
            } else if (expedition == null) { //Start an expedition
               cm.sendSimple("2083004_EXPEDITION_INTRO", expeditionName, em.getProperty("party"), expeditionBoss)

               status = 1
            } else if (expedition.isLeader(cm.getCharacterId())) { //If you're the leader, manage the expedition
               if (expedition.isInProgress()) {
                  cm.sendOk("2083004_EXPEDITION_ALREADY_IN_PROGRESS")
               ...
               if (expedition.contains(cm.getCharacterId())) { //If you're in it but it hasn't started, be patient
                  cm.sendOk("2083004_ALREADY_REGISTERED", expedition.getLeaderName())
               ...
               if (expedition.contains(cm.getCharacterId())) { //If you're registered, warp you in
                  ...
                  if (eim.getIntProperty("canJoin") == 1) {
                     ...
                     cm.sendOk("2083004_ALREADY_STARTED", expeditionBoss)
                  }

                  cm.dispose()
               } else { //If you're not in by now, tough luck
                  cm.sendOk("2083004_ANOTHER_EXPEDITION_HAS_STARTED", expeditionBoss)
                  cm.dispose()
               }
            }
         } else if (status == 1) {
            if (selection == 1) {
               expedition = cm.getExpedition(expeditionType)
               if (expedition != null) {
                  cm.sendOk("2083004_SOMEONE_ALREADY_LEADER")
                  cm.dispose()
                  return
               }
               ...
               if (res == 0) {
                  cm.sendOk("2083004_EXPEDITION_CREATED", expeditionBoss)
               } else if (res > 0) {
                  cm.sendOk("2083004_QUOTA_LIMIT")
               } else {
                  cm.sendOk("2083004_UNEXPECTED_ERROR")
               }

               cm.dispose()
            } else if (selection == 2) {
               cm.sendOk("2083004_NOT_UP_TO_CHALLENGE", expeditionBoss)
               cm.dispose()
            }
         } else if (status == 2) {
            if (selection == 1) {
               if (expedition == null) {
                  cm.sendOk("2083004_COULD_NOT_BE_LOADED")
                  cm.dispose()
                  return
               }
               ...
               if (size == 1) {
                  cm.sendOk("2083004_NEED_MORE_MEMBERS")
                  cm.dispose()
                  return
               }
               ...
               if (size < min) {
                  cm.sendOk("2083004_MINIMUM_PLAYERS", min)
                  cm.dispose()
                  return
               }

               cm.sendOk("2083004_GOOD_LUCK")
               status = 4
            } else if (selection == 3) {
               cm.sendBlueTextToMap("EXPEDITION_ENDED_BY", expedition.getLeaderName())
               cm.endExpedition(expedition)
               cm.sendOk("2083004_EXPEDITION_ENDED")
               cm.dispose()
            }
         } else if (status == 4) {
            if (em == null) {
               cm.sendOk("2083004_COULD_NOT_BE_INITIALIZED")
               cm.dispose()
               return
            }
            ...
            if (selection > 0) {
               ...
               cm.sendOk("2083004_YOU_HAVE_BANNED", banned.name())
               cm.dispose()
            } else {
               cm.sendSimple(list)
               status = 2
...
NPC2083004 getNPC() {
   if (!getBinding().hasVariable("npc")) {
      ...
      getBinding().setVariable("npc", new NPC2083004(cm: cm))
   }
   return (NPC2083004) getBinding().getVariable("npc")
}
