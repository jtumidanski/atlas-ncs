   ...
   def processNanaQuest() {
      if (cm.haveItem(questItems[nanaLoc], 50)) {
         if (cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk("9201027_THANK_YOU")
            return true
         } else {
            cm.sendOk("9201027_ETC_SPACE_NEEDED")
         }
      } else {
         cm.sendOk("9201027_GATHER_ME", questItems[nanaLoc])
      }
         ...
         if (status == 0) {
            ...
            if (!cm.haveItem(4031367 + nanaLoc, 1)) {
               if (cm.isQuestCompleted(100401 + nanaLoc)) {
                  state = 1
                  cm.sendAcceptDecline("9201027_DID_YOU_LOSE", questItems[nanaLoc])
                  ...
                  state = 0
                  cm.sendAcceptDecline("9201027_SEARCHING_FOR")
               }
            } else {
               cm.sendOk("9201027_DID_YOU_GET")
               cm.dispose()
            }
         } else if (status == 1) {
            if (state == 0) {
               cm.startQuest(100401 + nanaLoc)
               cm.sendOk("9201027_COLLECT", questItems[nanaLoc])
               cm.dispose()
            } else {
               processNanaQuest()
               cm.dispose()
