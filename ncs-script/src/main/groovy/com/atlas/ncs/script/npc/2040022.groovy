   ...
   def action(Byte mode, Byte type, Integer selection) {
      ...
      if (status == 0 && mode == 1) {
         ...
         if (selectedType == 0) { //What's a stimulator?
            cm.sendNext("2040022_STIMULATOR_INFO")
            cm.dispose()
         ...
         if (!cm.canHold(item, 1)) {
            cm.sendOk("2040022_NEED_FREE_SLOT")
            cm.dispose()
            return
         } else if (cm.getMeso() < cost) {
            cm.sendOk("2040022_ONLY_ACCEPT_MESO")
            cm.dispose()
            return
         } else {
            if (mats instanceof ArrayList && matQty instanceof ArrayList) {
         ...
         if (!complete) {
            cm.sendOk("2040022_MISSING_ITEMS")
         } else {
            if (mats instanceof ArrayList && matQty instanceof ArrayList) {
               for (int i = 0; i < mats.size(); i++) {
            ...
            if (stimulator) { //check for stimulator
               ...
               if (deleted != 0) {
                  ...
                  cm.sendOk("2040022_SUCCESS")
               } else {
                  cm.sendOk("2040022_STIMULATOR_ERROR")
               }
